...
class ContinualCollectXY(CCPatch):
    ...
    def reset_fruit(self):
        ...
        red_ids, blue_ids = obj_ids[:len(
            obj_ids)//2], obj_ids[len(obj_ids)//2:]
    ...
    def step(self, a):
        ...
        nx, ny = min(max(nx, self.min_x), self.max_x), min(
            max(ny, self.min_y), self.max_y)
...
class ContinualCollectRGB(ContinualCollectXY):
    ...
    def reset_fruit(self):
        ...
        self.episode_template = self.get_episode_template(
    ...
    def get_visualization_segment(self):
        if self.episode_template is not None:
            ...
            states = [self.generate_observation(
                coord, self.object_status, self.reds) for coord in state_coords]
...
def draw_pretty(state):
    ...
    red, green, blue, gray, black = np.array([255., 0., 0.]), np.array([0., 255., 0.]), \
        np.array([0., 0., 255.]), np.array(
            [128., 128., 128.]), np.array([0., 0., 0.])
    reddish, bluish = np.array([204., 121., 167.]), np.array([0., 158., 115.])
    light_g, light_b = np.array(
        [236., 236., 236.]), np.array([165., 165., 165.])
